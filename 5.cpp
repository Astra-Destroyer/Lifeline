...
int knapsackBacktrack(int W, vector<int>& wt, vector<int>& val, int n, int idx, int curVal, int 
curWeight) { 
...
f
 loat bound(Node u, int n, int W, vector<int>& wt, vector<int>& val) { 
if (u.weight >= W) 
return 0; 
f
 loat profit_bound = u.profit; 
int j = u.level + 1; 
...
cout << "Knapsack using Branch and Bound: " << knapsackBranchBound(W, wt, val, n) << 
endl; 
return 0; 
} 
