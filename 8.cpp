...
int tspDynamicProgrammingHelper(int pos, int mask, const vector<vector<int>>& dist, 
vector<vector<int>>& dp) { 
    ...
    for (int city = 0; city < N; city++) { 
        if ((mask & (1 << city)) == 0) { // If city has not been visited 
            int newCost = dist[pos][city] + tspDynamicProgrammingHelper(city, mask | (1 << 
city), dist, dp); 
...
int tspDynamicProgramming(const vector<vector<int>>& dist) { 
    ...
    return tspDynamicProgrammingHelper(0, 1, dist, dp); // Start from city 0, with only city 0 
visited 
} 
 
// Function to calculate the lower bound (cost) of the path being explored 
int calculateLowerBound(vector<vector<int>>& dist, vector<bool>& visited, int 
currentCost) { 
...
void tspBranchAndBound(vector<vector<int>>& dist, vector<bool>& visited, int 
currentCity, int visitedCount, int currentCost, int& minCost) { 
